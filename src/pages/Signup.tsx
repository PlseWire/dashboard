...
const Signup = () => {
  ...
  const getStrengthConfig = (score: number) => {
    switch (score) {
      case 1: return { label: "WEAK", color: "bg-destructive", width: "25%" };
      case 2: return { label: "FAIR", color: "bg-yellow-500", width: "50%" };
      case 3: return { label: "GOOD", color: "bg-primary", width: "75%" };
      case 4: return { label: "STRONG", color: "bg-primary shadow-[0_0_10px_rgba(32,0,234,0.5)]", width: "100%" };
      default: return { label: "NONE", color: "bg-transparent", width: "0%" };
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Security credentials dispatched. Verification required.");
      navigate("/verify-email");
    } catch (err: any) {
      console.error("Signup error details:", err);
      const errorMessage = err.errors?.map((e: any) => e.message).join(". ") || "Registration failed.";
  ...
  return (
    <div className="w-full max-w-xl p-8 bg-card border border-border animate-fade-in relative overflow-hidden rounded-2xl">
          ...
          {password && (
            <div className="space-y-1.5 pt-1">
              <div className="flex justify-between items-center px-0.5">
                <span className="text-[9px] font-quantico tracking-widest text-muted-foreground uppercase">Strength</span>
                <span className={`text-[9px] font-quantico tracking-widest uppercase ${strength === 1 ? 'text-destructive' : strength === 2 ? 'text-yellow-500' : 'text-primary'}`}>
                  {config.label}
                </span>
              </div>
        ...
        <div className="btn-wrapper mt-2">
          <button type="submit" className="uiverse-btn" disabled={pending}>
            <div className="txt-wrapper">
              {pending ? (
                <span className="txt-1">INITIALIZING...</span>
              ) : (
      ...
      <div className="mt-4 flex flex-col space-y-4">
        <div className="relative">
          ...
          <div className="relative flex justify-center text-[9px] uppercase tracking-widest">
            <span className="bg-card px-2 text-muted-foreground">Or register via</span>
          </div>
        </div>
      ...
      <div className="mt-8 text-center">
        ...
        >
          Existing Operator? <span className="underline underline-offset-4 decoration-primary/30">Sign in</span>
        </Link>
      </div>
    </div>
  );
